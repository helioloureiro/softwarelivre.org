...
var AudioJS = Class.extend({
  ...
  init: function(element, setOptions){
    ...
    if (this.options.controlsBelow) {
      _V_.addClass(this.box, "ajs-controls-below");
    }
  ...
  buildController: function(){
    ...
    /* Creating this HTML
      <ul class="ajs-controls">
        <li class="ajs-play-control ajs-play">
          <span></span>
        </li>
        <li class="ajs-progress-control">
          <ul class="ajs-progress-holder">
            <li class="ajs-load-progress"></li>
            <li class="ajs-play-progress"></li>
          </ul>
        </li>
        <li class="ajs-time-control">
          <span class="ajs-current-time-display">00:00</span><span> / </span><span class="ajs-duration-display">00:00</span>
        </li>
        <li class="ajs-volume-control">
          <ul>
    ...
    */
    ...
    this.controls = _V_.createElement("ul", { className: "ajs-controls" });
    // Add the controls to the audio's container
    this.audio.parentNode.appendChild(this.controls);

    // Build the play control
    this.playControl = _V_.createElement("li", { className: "ajs-play-control ajs-play", innerHTML: "<span></span>" });
    this.controls.appendChild(this.playControl);

    // Build the progress control
    this.progressControl = _V_.createElement("li", { className: "ajs-progress-control" });
    this.controls.appendChild(this.progressControl);

    // Create a holder for the progress bars
    this.progressHolder = _V_.createElement("ul", { className: "ajs-progress-holder" });
    this.progressControl.appendChild(this.progressHolder);

    // Create the loading progress display
    this.loadProgress = _V_.createElement("li", { className: "ajs-load-progress" });
    this.progressHolder.appendChild(this.loadProgress)

    // Create the playing progress display
    this.playProgress = _V_.createElement("li", { className: "ajs-play-progress" });
    this.progressHolder.appendChild(this.playProgress);

    // Create the progress time display (00:00 / 00:00)
    this.timeControl = _V_.createElement("li", { className: "ajs-time-control" });
  ...
  onPlay: function(event){
    this.playControl.className = "ajs-play-control ajs-pause";
    this.trackPlayProgress();
  },

  // When the audio is paused
  onPause: function(event){
    this.playControl.className = "ajs-play-control ajs-play";
  ...
  updateVolumeDisplay: function(){
    ...
    for(var i=0; i<6; i++) {
      if (i < volNum) {
        _V_.addClass(this.volumeDisplay.children[i], "ajs-volume-level-on")
      } else {
        _V_.removeClass(this.volumeDisplay.children[i], "ajs-volume-level-on");
      }
    }
  },
